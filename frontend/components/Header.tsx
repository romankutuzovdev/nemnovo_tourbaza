              ...
              <nav className="flex items-center gap-2 md:gap-2.5 lg:gap-3 xl:gap-4 flex-nowrap shrink-0 min-w-max">
                {nav.slice(1).map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="font-sans text-[9px] md:text-[10px] lg:text-[11px] xl:text-xs 2xl:text-sm font-semibold tracking-wide text-black/80 hover:text-black transition-colors whitespace-nowrap py-0.5 shrink-0"
                  >
                    {item.label}
                  </Link>
                ))}
